...
public class Silverfish : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (!Physics.BoxCast(transform.position, new Vector3(0.25f, 0.01f, 0.01f), transform.forward, transform.rotation, Time.deltaTime * speed+0.5f, blockmask) && Physics.Raycast(transform.position + transform.forward * (Time.deltaTime * speed + 0.4f), transform.up*-0.1f, 0.3f, walkmask))
        {
